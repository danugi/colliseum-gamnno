...
- Оптимизирована визуализация барьера (реже тики + меньше частиц), чтобы уменьшить лаги.
- Волны теперь состоят из смешанных типов мобов по сложности и спавнятся пачками, а не только зомби.
- Шлемы у мобов убраны, но они всё равно не горят на солнце (огнестойкость + принудительное тушение).
- Скелеты/бродяги теперь принудительно спавнятся как лучники (лук в основной руке).
- Пауки при спавне сразу получают цель из участников сессии и агрятся на игрока.
- Добавлен периодический ретаргет пауков в сессии, чтобы они не "теряли" агр в бою.
- Реализованы permission-ноды `colosseum.use` / `colosseum.admin` через fabric-permissions-api.
- Наградные книги теперь выдаются как реальные enchanted books (со `stored_enchantments`), а не просто переименованные.
- Вайп группы (все игроки мертвы в боевой фазе) теперь мгновенно завершает сессию поражением.
- На Hard/Hardcore у мобов добавлена снаряга по шансам (оружие и части брони).
- После завершения сессии добавлена очистка оставшихся мобов/босса, чтобы они не зависали на арене.
